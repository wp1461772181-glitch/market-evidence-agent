...
- 未完成：P2 媒体正文与 8-K 附件的真实研究引文生成；P3 的真实事件触发修订样本与价格更正回放；P4–P8 的联合模型、前端 V2 流程、调度切换及到期评估。旧证据修订仍保持原数值概率。
- 下一步：用已确认的来源做 P2 有界完整研究验证，积累足够可核对事件并推进 P4；P1 的任务框架尚未配置真实执行处理器，不能把研究报告或纯行情 artifact 当作联合预测。
...
| P2 动态证据集合 | in_progress | SEC 历史目录按页限量扫描、8-K 同 accession 99.1 附件优先读取并冻结来源/哈希/覆盖状态、可恢复的本地语料清单；真实 `--plan` 发现五股 396 条候选，保存 2 份官方快照。冻结事件、双时间边界、特征合同、缓存抽取、正反研究与引文定位已实现；AAPL/GOOGL 真实抽取 2 次，GOOGL 真实研究 1 次且重复抽取命中缓存。MSFT 真实 99.1 附件已通过正规库存路径读取并冻结；一份 NVDA 媒体摘录在隔离库中完成上传与冻结定位。两类新材料的真实研究引文尚未验证。详见 [证据试点记录](v2-evidence-audit.md)与[媒体样本试点](v2-media-sample-audit.md)。 |
...
- SEC 真实读取到一份 MSFT 8-K 的 99.1 附件，核对长度与哈希，并通过正规库存路径冻结为 observed V2 事件；尚未用它运行真实研究引文生成。详见[证据试点记录](v2-evidence-audit.md)。
